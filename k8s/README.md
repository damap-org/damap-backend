...
| hostname           | Hostname where the DAMAP frontend and backend will be reachable. For local testing use                                                                                                                                                                                                                         | localhost                             |
...
| person_services    | A prioritized list of external services DAMAP will use to resolve and enrich person (researcher) information.<br>Each entry includes:<ul><li>Human-readable name shown in the UI</li><li>Internal identifier for the service.class-name</li><li>Fully qualified Java class implementing the service.</li></ul> | [pure, orcid]                         |
...
💡 You should also review and adjust the resource requests and limits defined in the YAML manifests to ensure Keycloak has enough CPU and memory to start within the expected time.
