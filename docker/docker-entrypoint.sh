...
# Default DB kind is PostgreSQL, unless overridden by QUARKUS_DATASOURCE_DB_KIND.
: "${QUARKUS_DATASOURCE_DB_KIND:=postgresql}"

if [ "$QUARKUS_DATASOURCE_DB_KIND" = "postgresql" ]; then
    log "Using datasource configuration for PostgreSQL."
    export QUARKUS_DATASOURCE_JDBC_DRIVER="org.postgresql.Driver"
    export QUARKUS_DATASOURCE_JDBC_URL="jdbc:postgresql://${DAMAP_DB_HOST:-postgres-db}:${DAMAP_DB_PORT:-5432}/${DAMAP_DB_NAME:-damap}"
    export QUARKUS_HIBERNATE_ORM_DIALECT="org.hibernate.dialect.PostgreSQLDialect"
elif [ "$QUARKUS_DATASOURCE_DB_KIND" = "oracle" ]; then
    log "Using datasource configuration for Oracle."
