...
import org.damap.base.rda.dmpcommonstandard.DMPID;
...
import org.damap.base.rda.dmpcommonstandard.Dataset;
...
public class RdaDmpService {
    ...
    public DMPListResponseBody listDMPs(RdaDmpSearchParams params) {
        ...
        return new DMPListResponseBody()
                .totalCount(Math.toIntExact(filtered.size()))
                .items(items);
    }
    ...
    public RdaDmpResult createDMP(DMPDocument rdaDmpDocument) {
        ...
        try {
            ...
            return new RdaDmpResult(
                    toRdaDmp(createdDmp),
                    lastModified(createdDmp)
            );
        ...
        return dmp.getDatasets().stream()
                .anyMatch(dataset ->
                        dataset.getDatasetId() != null
                                && datasetIds.stream().anyMatch(id ->
                                equalsIgnoreCase(id, dataset.getDatasetId().getIdentifier())));
    }
        ...
        return dmp.getContributors().stream()
                .anyMatch(contributor ->
                        contributor.getPersonId() != null
                                && contributorIds.stream().anyMatch(id ->
                                equalsIgnoreCase(id, contributor.getPersonId().getIdentifier())));
    }
    ...
    private DMPWithID toRdaDmp(DmpDO dmp) {
        try {
            ...
            throw new InternalServerErrorException(
                    "Could not convert DAMAP DMP with id " + dmp.getId() + " to RDA format",
                    e
            );
        }
    }
        ...
        try {
            OffsetDateTime clientTimestamp =
                    java.time.ZonedDateTime
                            .parse(ifUnmodifiedSince, DateTimeFormatter.RFC_1123_DATE_TIME)
                            .toOffsetDateTime()
                            .withNano(0);

            Date serverDate =
                    existingDmp.getModified() != null
                            ? existingDmp.getModified()
                            : existingDmp.getCreated();
